...
class ChatStack(Construct):
    ...
    def __init__(self, scope: Construct, construct_id: str, database_stack, **kwargs) -> None:
        ...
        self.chat_function = _lambda.Function(
            ...
            environment={
                "QUOTA_TABLE_NAME": database_stack.quota_table.table_name,
                "PROGRESS_TABLE_NAME": database_stack.progress_table.table_name,
                "APTITUDES_TABLE_NAME": database_stack.aptitudes_table.table_name
            }
        )
        
        # Agregar permisos (automático)
        database_stack.quota_table.grant_read_write_data(self.chat_function)
        database_stack.progress_table.grant_read_write_data(self.chat_function)
        database_stack.aptitudes_table.grant_read_data(self.chat_function)
